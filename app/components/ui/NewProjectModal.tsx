...
export function NewProjectModal({ isOpen, onClose, onSave }: Props) {
  ...
  useEffect(() => {
    if (!isOpen) { setValues(empty); setErrors({}); setShowSuccess(false); setImageFile(null); setImagePreview(null) }
    else {
      // Fetch lookups
      const fetchLookups = async () => {
        const { clients, employees } = await getProjectOptions()
        setClients(clients)
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (values.crew.length && res.data?.id) {
        const crewRes = await setProjectCrew(res.data.id, values.crew)
      ...
      // Surface the real reason — a bare "failed" message hides schema/constraint errors.
  ...
  return (
    <>
      ...
      {showSuccess && <SuccessModal
          ...
          subtitle="Your project has been set up and is ready for tracking, updates, and team collaboration."
