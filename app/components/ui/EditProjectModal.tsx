...
import { X, CalendarDays, ChevronDown, Loader2, ImagePlus, Trash2 } from 'lucide-react'
import type { Project, ProjectStatus, ProjectType } from '@/app/admin/(portal)/projects/data'
import { getProjectOptions, getProjectCrew, setProjectCrew, updateProject, uploadProjectImage } from '@/app/admin/(portal)/projects/actions'
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <h3 className="text-sm font-semibold text-gray-900 mb-4">{children}</h3>
}
...
export function EditProjectModal({ project, onClose, onSave }: Props) {
  ...
  useEffect(() => {
    if (project) {
      ...
      const fetchLookups = async () => {
        const [{ clients, employees }, crew] = await Promise.all([
          getProjectOptions(),
          getProjectCrew(project.id),
        ])
        setClients(clients)
        setEmployees(employees)
        setValues((prev) => ({ ...prev, crew }))
      }
      fetchLookups()
    }
  }, [project])
  ...
  function handleImageChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // Caught here rather than after a slow round trip that fails at the bucket.
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const crewRes = await setProjectCrew(project.id, values.crew)
      if ('error' in crewRes) throw new Error(crewRes.error)

      onSave({ ...project, ...updateData } as unknown as Project)
      onClose()
    } catch (error) {
      console.error('Error updating project:', error)
      // Surface the real reason — a bare "failed" message hides upload and
      // schema errors the user could actually act on.
          ...
          <div className="overflow-y-auto flex-1 px-7 py-6 space-y-7">
            ...
            <div>
              ...
              <TeamAssignmentFields
                ...
                crew={values.crew}
                onManagerChange={(id) => set('manager_id', id)}
                onCrewChange={(ids) => set('crew', ids)}
              />
            </div>
            {/* Project Image */}
            <div>
              <FieldLabel>Project Image</FieldLabel>
              <input ref={imageInputRef} type="file" accept={IMAGE_ACCEPT} className="hidden" onChange={handleImageChange} />
              {imagePreview ? (
